...
var makeaccs = function(execute) {
  ...
  var output = {start:recs.hits.hits.length,created:0,groupadd:0,noted:0,end:0};
  for (var k in recs.hits.hits) {
    ...
    if ( !user ) {
      if (execute) {
        ...
        var apikey = rec.api_key;
        var apihash = Accounts._hashLoginToken(apikey);
        var userId = Accounts.createUser({email:email,password:password}); // TODO use old _id or store the mapping to it somehow?
  ...
  output.end = output.created + output.groupadd + output.noted;
